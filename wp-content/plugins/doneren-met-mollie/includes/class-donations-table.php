...
class Dmm_List_Table extends WP_List_Table {
    ...
    function column_default( $item, $column_name ) {
        switch( $column_name ) {
            case 'dm_amount':
                return ($item['payment_method'] ? '<img valign="top" src="https://www.mollie.com/images/payscreen/methods/' . $item['payment_method'] . '.png" width="18"> ' : '') . '&euro; ' . number_format($item[ $column_name ], 2, ',', '') . ' ' . (isset($item['customer_id']) && $item['customer_id'] ? '<small>(recurring)</small>' : '');
                break;
            case 'time':
